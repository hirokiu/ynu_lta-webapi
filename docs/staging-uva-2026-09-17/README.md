...
確認環境をAPI `4cfd207` / Web `481fa52`、イメージ `staging-4cfd207` に更新。更新前に確認用DBをバックアップし、全サービスhealthyを確認。旧本番・DNSは変更なし。

ChromeでSurvey一覧を10件へ切り替えて10行表示を確認。Surveyの新しい回答出力欄で10回答を選択し、次ページへ移動しても10件の選択が保持されること、選択CSVの生成完了、日本時間の2026-07-24～2026-07-26の期間指定CSV生成を確認。条件を変えると以前の保存リンクが破棄されることも確認。テスト用の選択・日付はブラウザー上で解除済み。
