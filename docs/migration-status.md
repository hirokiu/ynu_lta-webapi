...
3. 認証・アプリ更新：着手。devでGoogleログインとUID管理者設定を実装。本人による管理画面確認完了。UID対応、旧認証ドメイン除去、確認済み連絡メール・再設定・必要なアプリ更新。
...
2026-09-18：devにAPI 2be1cde / Web 06a6bf7を反映。UIDモードで管理者1名（hiroki_u）、未知UID拒否、未認証API401、ヘルス正常、通知falseを確認。本人Googleログインで管理画面へ遷移できることを利用者が確認済み。新サーバーのビルド時メモリ圧迫を受け、旧非公開stagingは停止（データ保持）。protoと旧本番は変更なし。dev確認URLはSSHトンネル経由のhttp://localhost:18082/login。

2026-09-18：上松よりdev管理画面へのログイン成功・問題なしとの確認。devの初期管理者認証は受入確認済み。次はproto既存利用者のFirebase UID対応調査と認証情報保全を進め、旧ドメイン除去を準備する。アプリ更新・proto認証移行・外部公開は未完了。
